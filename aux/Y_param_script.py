...
G_A          = ( abs(y_f)**2 * y_s.real ) / ( (y_o*y_s + y_o*y_i - y_r*y_f)*(y_i+y_s).conjugate() ).real
G_P          = ( (abs(y_f)**2) / (abs(y_o+y_L)**2) ) * (y_L.real)/(y_in.real)
G_T          = ( 4*y_s.real*y_L.real*abs(y_f)**2)/abs((y_s+y_i)*(y_o+y_L)-y_r*y_f)**2
k            = ( 2*(y_i.real+y_s.real)*(y_o.real+y_L.real) )/ ( (y_r*y_f).real + abs(y_r*y_f) )
...
print ("C       = " + str(C))
print ("beta*A  = " + str(beta_A) + " = " + str(beta_A_polar[0]) + "∠" + str(beta_A_polar[1]) + " rad")
print ("y_in    = " + str(y_in))
