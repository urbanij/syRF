...
def calculate_GP(s11, s12, s21, s22, zl, z0):
    ...
    return ((abs(s21)) ** 2 / (abs(1 - s22 * gamma_L)) ** 2) * (
        (1 - abs(gamma_L) ** 2) / (1 - abs(gamma_in) ** 2)
    )
...
def calculate_GT(s11, s12, s21, s22, zs, zl, z0):
    ...
    return ((1 - abs(gamma_S) ** 2) * (abs(s21) ** 2) * (1 - abs(gamma_L) ** 2)) / (
        (abs(1 - gamma_out * gamma_L) ** 2) * (abs(1 - s11 * gamma_S) ** 2)
    )
...
def calculate_GA(s11, s12, s21, s22, zs, z0):
    gamma_S = calculate_gamma(zs, z0)
    gamma_out = calculate_gamma_out(s11, s12, s21, s22, zs, z0)
    return ((abs(s21)) ** 2 / (abs(1 - s11 * gamma_S)) ** 2) * (
        (1 - abs(gamma_S) ** 2) / (1 - abs(gamma_out) ** 2)
    )
...
def calculate_NF(NFmin_db, Rn, gamma_s_on, zs, z0):
    ...
    if abs(gamma_S) == 1:
        return "inf"
    NF = NFmin + (4 * rn * abs(gamma_S - gamma_s_on) ** 2) / (
        (1 - abs(gamma_S) ** 2) * abs(1 + gamma_s_on) ** 2
    )
    return NF
...
def calculate_NF_circle(
    s11, s12, s21, s22, z0, NF_circle_dB, gamma_s_on, NF_opt_dB, Rn
):
...
def calculate_GA_circle(s11, s12, s21, s22, Ga_circle_dB):
    ...
    Ca = (ga * (s11 - s22.conjugate() * D).conjugate()) / (
        1 + ga * (abs(s11) ** 2 - abs(D) ** 2)
    )
    ra = ((1 - 2 * K * abs(s12 * s21) * ga + ga**2 * abs(s12 * s21) ** 2) ** 0.5) / (
        abs(1 + ga * (abs(s11) ** 2 - abs(D) ** 2))
    )
    return Ca, ra
...
def calculate_GP_circle(s11, s12, s21, s22, Gp_circle_dB):
    ...
    D = calculate_D(s11, s12, s21, s22)
    K = calculate_K(s11, s12, s21, s22)
    Cp = (gp * (s22 - s11.conjugate() * D).conjugate()) / (
        1 + gp * (abs(s22) ** 2 - abs(D) ** 2)
    )
    rp = ((1 - 2 * K * abs(s12 * s21) * gp + gp**2 * abs(s12 * s21) ** 2) ** 0.5) / (
        abs(1 + gp * (abs(s22) ** 2 - abs(D) ** 2))
    )
    return Cp, rp
...
def calculate_GTi(Gt_circle_dB, s11, s21, zs, z0):
    ...
    return (Gt_circle * abs(1 - s11 * gamma_S) ** 2) / (
        (1 - abs(gamma_S) ** 2) * abs(s21) ** 2
    )
...
def calculate_GT_circle(s11, s12, s21, s22, zs, zl, z0, Gt_circle_dB):
    ...
    rt = (
        (GTi**2 * abs(gamma_out) ** 2) / (GTi * abs(gamma_out) ** 2 + 1) ** 2
        + (1 - GTi) / (GTi * abs(gamma_out) ** 2 + 1)
    ) ** 0.5
    return Ct, rt
...
def calculate_gamma_S_opt(s11, s12, s21, s22):
    ...
    abs_gamma_S_opt = (
        B1 / (2 * abs(C1)) - ((B1**2 - 4 * abs(C1) ** 2) / (4 * abs(C1) ** 2)) ** 0.5
    )
...
def calculate_gamma_L_opt(s11, s12, s21, s22):
    ...
    abs_gamma_L_opt = (
        B2 / (2 * abs(C2)) - ((B2**2 - 4 * abs(C2) ** 2) / (4 * abs(C2) ** 2)) ** 0.5
    )
