...
def get_abscissa_from_first_graph(er, z):
    ...
    ind_ = np.where(f == closest_to_z)[0][
        0
    ]  # get the index of that value into the array f
...
def compute_matching_microstrip(self):
    ...
    try:
        lambda_over_lambda_TEM2 = get_ordinate_from_second_graph(
            epsilon_r_2, w_over_h_2
        )
    except Exception as e:
...
def show_plots(self):
    ...
    (
        ...
        x12_2,
        er12_2,
    ) = pl.calculate_epsilon_r_2()

    pl.visualize_z0_over_whratio(
        x1,
        er1,
        x2,
        er2,
