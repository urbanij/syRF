...
class mainProgram(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.distance_horizontalScrollBar.valueChanged.connect(
            self.Calculate_button.click
        )
        self.length_horizontalScrollBar.valueChanged.connect(
            self.Calculate_button.click
        )

        self.Calculate_button.clicked.connect(self.calculate_matching)
        self.Plot_impendace_admittance_button.clicked.connect(
            self.plot_impedance_admittance
        )
    ...
    def calculate_matching(self):
        ...
        d = float(
            self.distance_horizontalScrollBar.value() * RANGE_SCROLLBAR / MAX_SCROLLBAR
        )
        self.distance_lineedit.setText("{:.3f}".format(d))
        l = float(
            self.length_horizontalScrollBar.value() * RANGE_SCROLLBAR / MAX_SCROLLBAR
        )
        ...
        try:
            Zv1 = (
                Z0
                * (ZL - 1j * Z0 * math.tan(2 * math.pi * -d))
                / (Z0 - 1j * ZL * math.tan(2 * math.pi * -d))
            )
        except Exception as e:
            Zv1 = msg_error
        ...
        try:
            ...
            if self.series_stub_radioButton.isChecked():
                if (
                    abs(zv2.real - 1) <= THRESHOLD_OK_VAL
                    and abs(zv2.imag) <= THRESHOLD_OK_VAL
                ):
        ...
        try:
            if abs(zv2) < 1e-14:
                ...
                if self.parallel_stub_radioButton.isChecked():
                    if (
                        abs(yv2.real - 1) <= THRESHOLD_OK_VAL
                        and abs(yv2.imag) <= THRESHOLD_OK_VAL
                    ):
    ...
    def plot_impedance_admittance(self):
        ...
        Z = (
            Z0
            * (ZL - 1j * Z0 * np.tan(2 * np.pi * z))
            / (Z0 - 1j * ZL * np.tan(2 * np.pi * z))
        )
        if self.parallel_stub_radioButton.isChecked():
            plt.title(
                "y(z) @ $Z_L$={:.3g} $\Omega$, $Z_0$={:.2g} $\Omega$".format(ZL, Z0)
            )
            plt.plot(
                z, (1 / (Z / Z0)).real, label="Re{y(z)}", color="red", linewidth=0.8
            )
            plt.plot(
                z, (1 / (Z / Z0)).imag, label="Im{y(z)}", color="blue", linewidth=0.8
            )
            plt.plot(
                z,
            ...
            plt.title(
                "z(z) @ $Z_L$={:.3g} $\Omega$, $Z_0$={:.2g} $\Omega$".format(ZL, Z0)
            )
