...
class SmithChart(object):
    ...
    def _impedance_circle_coords(self, intercept_x_coords, intercept_angles):
        ...
        xs = np.sin(intercept_angles) / (
            1 - np.cos(intercept_angles)
        )  # convert to desired reactances
        x_circle_coords = [((1, 1 / x), abs(1 / x)) for x in xs]

        return r_circle_coords, x_circle_coords, rs, xs

    def draw_impedance_circles(
        self, intercept_x_coords, intercept_angles, labels=False
    ):
        r_circle_coords, x_circle_coords, rs, xs = self._impedance_circle_coords(
            intercept_x_coords, intercept_angles
        )
        ...
        if labels:
            for x, r in zip(intercept_x_coords, rs):
                ...
                self.ax.text(
                    x + 0.04, 0.03, "%s" % str(self.z0 * r), **self.label_options
                )
    ...
    def draw_chart_axes(self):
        """make outer circle"""
        self.smith_circle = Circle(
            (0, 0), 1, transform=self.ax.transData, fc="none", **self.patch_options_axis
        )
    ...
    def draw_smith_chart(self, admittance, labels):
        ...
        self.patch_options_light = {
            "fc": "none",
            "color": "#474959",
            "alpha": 0.45,
            "lw": 0.2,
        }
        self.patch_options_dark = {
            "fc": "none",
            "color": "#474959",
            "alpha": 0.8,
            "lw": 0.3,
        }
        self.patch_options_axis = {"color": "black", "alpha": 1, "lw": 0.4}
    ...
    def plot_circle(
        ...
        if (x**2 + y**2) == 1:
            ...
            textpos = (
                x / dist_to_circle + text_x_offset,
                y / dist_to_circle + text_y_offset,
            )
        else:
        ...
        self.ax.annotate(
            ...
            xytext=textpos,
            arrowprops=dict(
                arrowstyle="->", alpha=text_alpha, connectionstyle="arc3,rad=0.17"
            ),
        )
...
def plot_Smith(
    ...
    circle_in = plt.Circle((Cs.real, Cs.imag), rs, color="red", fill=True, alpha=0.15)
    plot(
        Cs.real, Cs.imag, marker=None, color="#fbe2e1", label="Input stability circle"
    )  # fbe2e1 is red with alpha=0.15

    circle_out = plt.Circle(
        (Cl.real, Cl.imag), rl, color="green", fill=True, alpha=0.15
    )
    plot(
        Cl.real, Cl.imag, marker=None, color="#CCFFCC", label="Output stability circle"
    )  # CCFFCC is green with
    ...
    if gamma_S != msg_error:
        if label_gamma_inout:  # if true shows gamma as label
            ...
            plot(
                ...
                marker="o",
                color="coral",
                label="$\Gamma_{S}(Z_S=" + str(ZS) + "\Omega)$",
            )
        # draw circumference centered in the origin passing from gamma_S point
        gamma_S_circumference = plt.Circle(
            (0, 0), abs(gamma_S), color="coral", linewidth=0.3, fill=False
        )
    ...
    if gamma_in != msg_error:
        if label_gamma_inout:  # if true shows gamma as label
            plot(
                ...
                marker="o",
                color="crimson",
                label="$\Gamma_{in}(\Gamma_L=" + str(gamma_L_visualized) + ")$",
            )
        else:
            plot(
                gamma_in.real,
                ...
                marker="o",
                color="crimson",
                label="$\Gamma_{in}(Z_L=" + str(ZL) + "\Omega)$",
            )
    ...
    if gamma_L != msg_error:
        if label_gamma_inout:  # if true shows gamma as label
            ...
            plot(
                ...
                marker="o",
                color="lime",
                label="$\Gamma_{L}(Z_L=" + str(ZL) + "\Omega)$",
            )
        # draw circumference centered in the origin passing from gamma_L point
        gamma_L_circumference = plt.Circle(
            (0, 0), abs(gamma_L), color="lime", linewidth=0.3, fill=False
        )
    ...
    if gamma_out != msg_error:
        if label_gamma_inout:  # if true shows gamma as label
            plot(
                ...
                marker="o",
                color="darkgreen",
                label="$\Gamma_{out}(\Gamma_S=" + str(gamma_S_visualized) + ")$",
            )
        else:
            plot(
                gamma_out.real,
                ...
                marker="o",
                color="darkgreen",
                label="$\Gamma_{out}(Z_S=" + str(ZS) + "\Omega)$",
            )
    ...
    if constant_gamma_circle != msg_error:
        ...
        plot(
            ...
            marker=None,
            color="#00CCCC",
            label="$\\bar \Gamma$ = " + str(constant_gamma_circle),
        )
        ax.add_artist(Gamma)
...
def plot_Smith_smith_matching(Z0, Z_in, Z_out, gamma_zin, gamma_zout):
    ...
    if gamma_zin != msg_error:
        ...
        plot(
            ...
            color="red",
            alpha=alpha_z_point,
            label=gamma_zin_label,
        )

        """ constant resistance arc """
        (_cx, _cy), _r = equi_resistance_circle(gamma_zin)
        resistance_zin_circumference = plt.Circle(
            (_cx, _cy),
            _r,
            color="red",
            linestyle="--",
            alpha=alpha_resistance_and_reactance_arcs,
            linewidth=lw_resistance_reactance_arcs,
            fill=False,
        )
        ...
        reactance_zin_circumference = plt.Circle(
            (_cx, _cy),
            _r,
            color="red",
            linestyle="-.",
        ...
        (_cx, _cy), _r = equi_reactance_circle(-gamma_zin)
        susceptance_zin_circumference = plt.Circle(
            (_cx, _cy),
            _r,
            color="red",
            linestyle="-.",
        ...
        gamma_zin_circumference = plt.Circle(
            (0, 0),
            abs(gamma_zin),
            color="red",
            linestyle="-",
            alpha=alpha_constant_gamma,
            linewidth=lw_constant_gamma,
            fill=False,
        )
    ...
    if gamma_zout != msg_error:
        ...
        reactance_zout_circumference = plt.Circle(
            (_cx, _cy),
            _r,
            color="blue",
            linestyle="-.",
        ...
        """ constant susceptance arc """
        (_cx, _cy), _r = equi_reactance_circle(-gamma_zout)
        susceptance_zout_circumference = plt.Circle(
            (_cx, _cy),
            _r,
            color="blue",
            linestyle="-.",
        ...
        """ constant gamma circumference"""
        gamma_zout_circumference = plt.Circle(
            (0, 0),
            abs(gamma_zout),
            color="blue",
            linestyle="-",
    ...
    subplots_adjust(
        left=-0.01, bottom=-0.01, right=1.005, top=1.005, wspace=0.45, hspace=0.2
    )
    show()
