...
__all__ = ["OnePort", "TwoPort", "_11", "_12", "_21", "_22", "plot_gains"]
    ...
    def __repr__(self):
        ...
        if len(self.s) == 1:
            ...
            if abs(z.imag) > 0:
                ...
                details += " = {0:.2f}//{1:.2f}j (".format(
                    float(rp.real), float(xp.imag)
                )
...
class TwoPortBase(NPort):
    ...
    def _from_y(self, y_params):
        ...
        self.s_params = array(
            [dot((I - self.z0 * yi), inv(I + self.z0 * yi)) for yi in y_params]
        )

    def _from_z(self, z_params):
        z_params = asarray(z_params, dtype="complex").reshape(-1, 2, 2)
        self.s_params = array(
            [(zi - self.z0 * I).dot(inv(zi + self.z0 * I)) for zi in z_params]
        )
    ...
    def _to_z(self):
        return (
            array([inv(I - si).dot(I + si) for si in self.s_params.reshape(-1, 2, 2)])
            * self.z0
        )
    ...
    def interpolate(self, new_points):
        ...
        for param in (_11, _12, _21, _22):
            interp_real = interpolate.InterpolatedUnivariateSpline(
                self.f, s[param].real
            )
            interp_imag = interpolate.InterpolatedUnivariateSpline(
                self.f, s[param].imag
            )
    ...
    def __repr__(self):
        ...
        if (self.mu_s() < 1).any():
            details += "potentially unstable (k_min = {}), ".format(
                self.rollett_kt().min()
            )
        else:
            g = self.g_max()
...
class TwoPort(TwoPortBase):
    ...
    def transducer_gain(self, source=None, load=None):
        ...
        return (
            ((1 - abs(ss) ** 2) / abs(1 - self.s_in(load) * ss) ** 2)
            * abs(self.s[_21]) ** 2
            * ((1 - abs(sl) ** 2) / abs(1 - self.s[_22] * sl) ** 2)
        )
    ...
    def available_gain(self, source=None):
        ...
        return (
            ((1 - abs(ss) ** 2) / abs(1 - s[_11] * ss) ** 2)
            * abs(s[_21] ** 2)
            / (1 - abs(self.s_out(source)) ** 2)
        )
    ...
    def power_gain(self, load=None):
        ...
        return (
            ((1 - abs(sl) ** 2) / abs(1 - s[_22] * sl) ** 2)
            * abs(s[_21] ** 2)
            / (1 - abs(self.s_in(load)) ** 2)
        )
    ...
    def mu_stability_source(self):
        ...
        return (1 - abs(s[_11]) ** 2) / (
            abs(s[_12] * s[_21]) + abs(s[_22] - d * (s[_11].conj()))
        )
    ...
    def mu_stability_load(self):
        ...
        return (1 - abs(s[_22]) ** 2) / (
            abs(s[_12] * s[_21]) + abs(s[_11] - d * (s[_22].conj()))
        )
    ...
    def rollett_k(self):
        ...
        return (1 - abs(s[_11]) ** 2 - abs(s[_22]) ** 2 + abs(d) ** 2) / (
            2 * abs(s[_12] * s[_21])
        )
    ...
    def operating_gain_circle(self, G):
        ...
        radius = np.sqrt(
            g**2 * abs(s[_12] * s[_21]) ** 2 - 2 * g * k * abs(s[_12] * s[_21]) + 1
        ) / abs(1 + g * d)
    ...
    def available_gain_circle(self, G):
        ...
        center = g * (c.conj()) / (1 + g * d)
        radius = np.sqrt(
            (g * abs(s[_12] * s[_21])) ** 2 - 2 * g * k * abs(s[_12] * s[_21]) + 1
        ) / abs(1 + g * d)
...
class Cascade(TwoPort):
    def __init__(self, left_port, right_port, *args, **kwargs):
        ...
        self.description = "Cascade of {} two-port networks".format(
            len(self.components)
        )
