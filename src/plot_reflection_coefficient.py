...
def plot_gamma(rv):
	...
	if X_L >=0:
		...
		if R_L >= R_0:
			...
			z0_2 = 1/(1j*w*C2) + 1/((1/(1j*w*L2) + 1/(R_L+1j*w*X_L/w0)))

		else:
			# up conversion
			z0_1 = 1/(1j*w*C2 + 1/(1j*w*L2 + R_L + 1j*w*X_L/w0))
			z0_2 = 1/(1/(1j*w*L1) + 1/(1/(1j*w*C1) + R_L + 1j*w*X_L/w0))
		...
		if R_L >= R_0:
			# down conversion
			z0_1 = 1j*w*L1 + 1/(1j*w*C1 + 1/(R_L + 1/(1j*w*1/(w0*X_L))))
			z0_2 = 1/(1j*w*C2) + 1/((1/(1j*w*L2) + 1/(R_L+1/(1j*w*1/(w0*X_L)))))

		else:
			# up conversion
			z0_1 = 1/(1j*w*C2 + 1/(1j*w*L2 + R_L + 1/(1j*w*1/(w0*X_L))))
			z0_2 = 1/(1/(1j*w*L1) + 1/(1/(1j*w*C1) + R_L +1/(1j*w*1/(w0*X_L))))
